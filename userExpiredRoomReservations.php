...
$exp_room_res_result = $mysqli->query("SELECT `reservation`.*, `users`.*, `rooms`.*, l.name lname
                FROM `reservation` 
...
<a style="margin: auto;" class="link-btn" href="userReservations.php">Back to upcoming reservations</a>
<div class='reservation-container' style='padding: 20px'>
    <div class="row">
        <?php
        echo("<h1>Past reservations</h1>");
        if ($exp_room_res_result->num_rows==0){
            $_SESSION['message'] = "You dont have any past reservations.";
            require_once 'utils.php';
            redirect('error.php');
        }
        while ($row = $exp_room_res_result->fetch_assoc()) {
